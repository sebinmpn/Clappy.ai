...
public:
    ...
    void deSelectColumn(int column) {
      digitalWrite(_colPins[column], LOW);
  }
...
#define ROWS 10
#define COLS 22
#define TOTAL_BITS 220  // 10x22 matrix
#define TOTAL_BYTES 28  // 27 data bytes + 1 'S' start byte
...
int set_char()
{
  ...
  for (int col = 0;  col < COLS; col++)
  {
    for (int row = 0; row < ROWS; row++)
    {
      ...
      {
        matrix.selectRow (row);
      }
      else
      {
        matrix.deSelectRow (row);
      }
    } 
    matrix.selectColumn (col);
    delayMicroseconds(200);
    matrix.deSelectColumn (col);
  }
}



void char_to_matrix(char command[2]) {

  byte receivedByte = command[1] - '0';  // Read packed byte
  bool boolArray[8];
  for (int i = 0; i < 8; i++) {
      boolArray[i] = (receivedByte >> (7 - i)) & 1;  // Extract each bit
  }

  // Print unpacked boolean values
  Serial.print("Unpacked Booleans: ");
  for (int i = 0; i < 8; i++) {
      Serial.print(boolArray[i]);
      Serial.print(" ");

      boolArray[i] ? ledMatrix[i][0] = 1 : ledMatrix[i][0] = 0;
  }
  Serial.println();
}


// void string_to_matrix(char command[4]) 
// {
//   int bitIndex = 0;
//   for(int i = 1; i < 4; i++)
//   {
//     byte receivedByte = command[i] - '0';  // Read packed byte
...
//     int col = (k - 1) / ROWS;
//     int row = (k - 1) % ROWS;
//     Serial.print(ledMatrix[col][row]);
//     Serial.print(" ");
...
void string_to_matrix(char command[TOTAL_BYTES]) 
{
  int bitIndex = 0;
        ...
        if (command.length() > 0) {
            ...
            else if (action == 'W' && command.length() == 5)
            {
              for (int i = 0; i < 4; i++) {
                int row = 0 + i / 2;
                int col = 0 + i % 2;
                ledMatrix[col][row] = command[i + 1] - '0';  // Convert char to int
            ...
            else if (action == 'W' && command.length() == 2)
            {
              char_to_matrix(command.c_str());
            ...
            else if (action == 'W' && command.length() == TOTAL_BYTES)
            {
              string_to_matrix(command.c_str());
